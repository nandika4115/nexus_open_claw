...
export function createSessionSnapshotBehavior(services: SkillServices): PlannedBehavior {
  return {
    ...
    handler: async () => {
      ...
      const tabMatch = matchBestThread(
        candidates,
        tabs.map((tab) => `${tab.title} ${tab.url}`).join(" ")
      );
      const fileMatch = matchBestThread(
        candidates,
        fileEvents.map((event) => event.path).join(" ")
      );
      ...
      if (tools.llm) {
        ...
        const updated = replaceSection(
          replaceSection(existingThread || `# Thread: ${activeThread.title}\n`, "Summary", summary.trim()),
          "Current Focus",
          summary.trim()
        );
