...
export function createConnectionEngineBehavior(services) {
    return {
        name: "connection_engine",
        handler: async () => {
            const { memory, tools, logger, config } = services;
            const threshold = config.runtimeConfig?.behaviors.connection_engine.similarity_threshold ?? 0.75;
            const threads = await memory.index.listThreads();
            const existing = await memory.connections.load();
            const existingPairs = new Set(existing.connections.map((conn) => `${conn.from_thread}:${conn.to_thread}`));
            for (let i = 0; i < threads.length; i += 1) {
                ...
                for (let j = i + 1; j < threads.length; j += 1) {
                    ...
                    const keywordOverlap = a.topic_keywords.filter((keyword) => b.topic_keywords.map((item) => item.toLowerCase()).includes(keyword.toLowerCase()));
                    const keywordScore = Math.min(0.3, keywordOverlap.length * 0.1);
                    let semanticScore = 0;
                    let sharedConcepts = keywordOverlap;
                    if (tools.llm) {
