...
export function createThreadResurrectionBehavior(services) {
    return {
        ...
        handler: async () => {
            ...
            const match = matchBestThread(candidates, `${tabs.map((tab) => `${tab.title} ${tab.url}`).join(" ")} ${fileEvents
            ...
            const updated = { ...thread, status: "active", last_touched: toIsoTimestamp() };
