...
async function main() {
    ...
    const channels = {
        ...
        slack: config.env.SLACK_BOT_TOKEN && config.env.SLACK_USER_ID
        ...
        email: config.env.SMTP_HOST && config.env.SMTP_USER && config.env.SMTP_PASS && config.env.EMAIL_RECIPIENTS
            ? new EmailChannel({
    ...
    engine.registerBehavior("thread_resurrection", createThreadResurrectionBehavior(skillServices).handler);
    ...
    gateway.start();
    const heartbeat = new HeartbeatDaemon(logger, config.memoryPath, {
